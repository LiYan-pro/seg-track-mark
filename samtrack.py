...
# ==============确保打包成功=================
import sys
sys.path.append('D:\\code\\samt_v01\\aot\\networks')
sys.path.append('D:\\code\\samt_v01\\aot\\networks\\decoder')
sys.path.append('D:\\code\\samt_v01\\aot\\networks\\encoder')
sys.path.append('D:\\code\\samt_v01\\aot\\networks\\encoder\\resnet')
sys.path.append('D:\\code\\samt_v01\\aot\\networks\\encoder\\swin')
sys.path.append('D:\\code\\samt_v01\\aot\\networks\\engines')
sys.path.append('D:\\code\\samt_v01\\aot\\networks\\layers')
sys.path.append('D:\\code\\samt_v01\\aot\\networks\\managers')
sys.path.append('D:\\code\\samt_v01\\aot\\networks\\models')
sys.path.append('D:\\code\\samt_v01\\aot\\tools')
sys.path.append('D:\\code\\samt_v01\\sam\\segment_anything')
sys.path.append('D:\\code\\samt_v01\\sam\\segment_anything\\modeling')
sys.path.append('D:\\code\\samt_v01\\sam\\segment_anything\\utils')
sys.path.append('D:\\code\\samt_v01\\sam\\tool')
...
input_video = f'{os.path.join(os.path.abspath(__file__))}/../input/test.mp4'
filename = 'test.mp4'
output_dir = f'{os.path.join(os.path.abspath(__file__))}/../results/ROI'
...
# 创建sam模型
sam = sam_model_registry["vit_b"](checkpoint=f'{os.path.join(os.path.abspath(__file__))}/../ckpt/sam_vit_b_01ec64.pth')
_ = sam.to(device="cuda")  # 注释掉这一行，会用cpu运行，速度会慢很多
predictor = SamPredictor(sam)  # SAM计算图像embedding


# 鼠标点击函数，运用opencv的鼠标函数
...
# 赋予颜色到mask上，展示到图像
def apply_color_mask(image, mask, color, color_dark=0.5):  # 对mask进行赋予颜色
    for c in range(3):
...
from segtrack_interface import *
image_orign,_,_ = get_meta_from_video(input_video)
...
while True:
    ...
    while True:
        ...
        display_info = f'test ' \
                        ...
                        f'| Press q to remove last point | Press space to clear '
        cv2.putText(image_display, display_info, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2, 
                    cv2.LINE_AA)
        """
        display_info = f'Track-Mark project \n' \
                       f'| Press w to choice object | press s to start track | Press Esc to exit |\n' \
                       f'| Press space to clear all points | Press q to remove last point |'
        y0_c, dy_c = 28, 28

        for i_c, txt_c in enumerate(display_info.split('\n')):
            y_c = y0_c + i_c * dy_c
            cv2.putText(image_display, txt_c, (10, y_c), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)
            ...
            if len(input_point) > 0 and len(input_label) > 0:
                ...
                while (1):
                    ...
                    mask_info = f'Total: {num_masks} | Current: {mask_idx} | Score: {scores[mask_idx]:.2f} \n' \
                        ...
                        f'| Press space to clear all points | Press q to remove last point |'
                    y0_m, dy_m = 28, 28
                    for i_m, txt_m in enumerate(display_info.split('\n')):
                        y_m = y0_m + i_m * dy_m
                        cv2.putText(image_display, txt_m, (10, y_m), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2,
                                    cv2.LINE_AA)
                    ...
                    if key == ord('q') and len(input_point) > 0:
                        ...
                        from segtrack_interface import seg_track
                        seg_track(input_point, input_label, input_video)

                        from keypointMarker import keypoint_marker
                        keypoint_marker()
