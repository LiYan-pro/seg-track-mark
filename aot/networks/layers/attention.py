...
# from aot.networks.layers.basic import DropOutLogit, DWConv2d
...
class MultiheadLocalAttentionV2(nn.Module):
    def __init__(self,
        ...
        if enable_corr:
            import sys
            sys.path.append("D:\\miniconda\\miniconda3\\envs\\samt\\Lib\\site-packages\\spatial_correlation_sampler-0.4.0-py3.9-win-amd64.egg\\")
            from spatial_correlation_sampler import SpatialCorrelationSampler
            self.correlation_sampler = SpatialCorrelationSampler(
                kernel_size=1,
                patch_size=self.window_size,
